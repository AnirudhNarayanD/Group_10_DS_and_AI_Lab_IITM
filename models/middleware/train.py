"""Middleware training script: dual-head loss (CrossEntropy + BCE safety gate).
...
def _compute_class_weights(train_records: Sequence[dict], device: torch.device) -> tuple:
    ...
    weight_details = {
        ...
        "justification": (
            "Inverse-frequency weighting ensures the loss function penalizes errors "
            "on minority classes (harmful, jailbreak) proportionally more than on the "
            "majority class (benign). This prevents the model from achieving high accuracy "
            "by simply predicting the majority class, which is critical for a guardrail "
            "system where missing an attack is more costly than a false alarm."
        ),
    }
...
def _evaluate(model: GuardrailClassifier, loader: DataLoader, device: torch.device) -> dict:
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            safety_scores_all.extend(output.safety_score.squeeze(-1).cpu().tolist())
    ...
    if jailbreak_mask.sum() > 0:
        jailbreak_missed = ((labels_array[jailbreak_mask] == jailbreak_idx) & (preds_array[jailbreak_mask] != jailbreak_idx)).sum()
        jailbreak_asr = float(jailbreak_missed / jailbreak_mask.sum())
    harmful_asr = 0.0
    if harmful_mask.sum() > 0:
        harmful_missed = ((labels_array[harmful_mask] == harmful_idx) & (preds_array[harmful_mask] != harmful_idx)).sum()
    ...
    return {
        ...
        "safety_gate_avg_score": round(float(np.mean(safety_scores_all)) if safety_scores_all else 0.0, 4),
    }


def run_middleware_training(config: MiddlewareTrainConfig) -> dict:
    ...
    arch_cfg = ArchConfig(
        ...
        safety_gate_hidden=config.safety_gate_hidden,
        enable_safety_gate=config.enable_safety_gate,
    )
    ...
    criterion_safety = torch.nn.BCELoss()
    ...
    training_setup = {
        "model_type": "middleware",
        "model_name": config.model_name,
        "architecture": "Encoder \u2192 MiddleBlock \u2192 Decoder + SafetyGate",
        ...
        "loss_function": "CrossEntropyLoss (class-weighted) + BCE (safety gate)",
        "safety_gate_weight": config.safety_gate_weight,
        "seed": config.seed,
        "train_samples": len(train_records),
        "val_samples": len(val_records),
        "device": str(device),
        "class_weights": weight_details,
        "arch_config": asdict(arch_cfg),
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"MIDDLEWARE Training on {str(device).upper()} | {len(train_records)} train, {len(val_records)} val", flush=True)
    print(f"Model: {config.model_name} | Epochs: {config.epochs} | Batch: {config.batch_size} | LR: {config.learning_rate}", flush=True)
    print(f"Architecture: MiddleBlock({config.intermediate_size}) \u2192 Decoder({config.decoder_hidden_size}) + SafetyGate({config.safety_gate_hidden})", flush=True)
    ...
    for epoch in range(config.epochs):
        ...
        for batch in train_loader:
            ...
            safety_targets = (labels != LABEL_TO_ID["benign"]).float().unsqueeze(-1)
            loss_safety = criterion_safety(output.safety_score, safety_targets)
            loss = loss_cls + config.safety_gate_weight * loss_safety
        ...
        print(
            ...
            f"| safety_gate_avg: {val_metrics.get('safety_gate_avg_score', 0):.4f}",
            flush=True,
        )
        ...
        if val_metrics["macro_f1"] > best_val_macro_f1:
            ...
            patience_counter += 1
            if patience_counter >= config.early_stopping_patience and not config.smoke_mode:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train middleware DistilBERT guardrail classifier")
    ...
    parser.add_argument("--safety-gate-hidden", type=int, default=128)
    parser.add_argument("--enable-safety-gate", action="store_true", default=True)
    parser.add_argument("--disable-safety-gate", dest="enable_safety_gate", action="store_false")
    parser.add_argument("--safety-gate-weight", type=float, default=0.3)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    cfg = MiddlewareTrainConfig(
        ...
        safety_gate_hidden=args.safety_gate_hidden,
        enable_safety_gate=args.enable_safety_gate,
        safety_gate_weight=args.safety_gate_weight,
    )
    result = run_middleware_training(cfg)
    print("Middleware training completed.")
